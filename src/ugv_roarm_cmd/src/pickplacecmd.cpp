...
public:
  ...
  : Node("hand_publisher_node")
  {
    ...
    if (model == "roarm_m2") {
      last_point_ = {0.0, -0.7, 2.618};
    } else if (model == "roarm_m3") {
      last_point_ = {0.0, 0.0, 2.618, -1.0472, 0.0};
    }

    RCLCPP_INFO(this->get_logger(), "HandPublisherNode initialized with model: %s", model.c_str());
  }
  ...
  void publishTrajectory()
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Only one point, adding last_point as start.");
    traj_points_.insert(traj_points_.begin(), {last_point_, 0.0, traj_points_[0].max_speed});

    RCLCPP_INFO(this->get_logger(), "==== traj_points_ ====");
    for (size_t i = 0; i < traj_points_.size(); ++i) {
      const auto &p = traj_points_[i];
      std::ostringstream ss;
      ss << "  [" << i << "] duration=" << p.duration
        << ", max_speed=" << p.max_speed
        << ", joints=(";
      for (size_t j = 0; j < p.joints.size(); ++j) {
        ss << std::fixed << std::setprecision(3) << p.joints[j];
        if (j + 1 < p.joints.size()) ss << ", ";
      }
      ss << ")";
    ...
    traj.header.stamp = this->get_clock()->now() + rclcpp::Duration::from_seconds(0.1);
    ...
    for (size_t seg = 0; seg < traj_points_.size() - 1; ++seg) {
      ...
      if (p0.joints.size() != p1.joints.size()) {
        RCLCPP_ERROR(this->get_logger(),
                    "Segment #%zu joint dimension mismatch (%zu vs %zu)",
                    seg, p0.joints.size(), p1.joints.size());
        return;
      }
      ...
      if (dist < 1e-6) {
        RCLCPP_WARN(this->get_logger(), "Segment #%zu skipped (distance too small)", seg);
        continue;
      }

      // ===== ✅ 自动限速逻辑 =====
      double dt = (p1.duration > 0.0) ? p1.duration : (dist / p1.max_speed);
      // double dt = (p0.duration > 0.0) ? p0.duration : (dist / p0.max_speed);

      // 检查每个关节的速度是否超过 max_speed
      for (size_t j = 0; j < p0.joints.size(); ++j) {
        double delta = std::abs(p1.joints[j] - p0.joints[j]);
        double v = delta / dt;
        if (v > p1.max_speed) {
          double scale = v / p1.max_speed;
          dt *= scale;
          RCLCPP_WARN(this->get_logger(),
                      "Segment #%zu joint[%zu] exceeds speed (%.3f>%.3f), "
                      "increasing duration to %.3fs",
                      seg, j, v, p1.max_speed, dt);
        }
      }
      // ===== ====================

      if (dt < 1e-3) {
        RCLCPP_WARN(this->get_logger(), "Segment #%zu duration too small (%.6f), clamped", seg, dt);
        dt = 1e-3;
      }

      int steps = std::max(10, static_cast<int>(dt * 10)); // 10Hz 插值
      RCLCPP_INFO(this->get_logger(),
                  "Segment #%zu: dist=%.4f, duration=%.3fs, steps=%d",
                  seg, dist, dt, steps);

      for (int i = 0; i <= steps; ++i) {
        double t = dt * i / steps;
        double tau = std::clamp(t / dt, 0.0, 1.0);
        ...
        for (size_t j = 0; j < p0.joints.size(); ++j) {
          // RCLCPP_INFO(this->get_logger(),"p0 position = %.3f ", p0.joints[j]);
          // RCLCPP_INFO(this->get_logger(),"p1 position = %.3f ", p1.joints[j]);
          ...
          if (std::isnan(p.positions[j]) || std::isinf(p.positions[j]) ||
              std::isnan(p.velocities[j]) || std::isinf(p.velocities[j])) {
            RCLCPP_ERROR(this->get_logger(),
                        "Invalid joint value detected at segment #%zu, joint[%zu]: "
                        "pos=%.6f, vel=%.6f", seg, j, p.positions[j], p.velocities[j]);
            return;
          }
        }
        RCLCPP_INFO(this->get_logger(),
          "  point[%d]: time_from_start = %.3f s", i, t_cumulative + t);
        p.time_from_start = rclcpp::Duration::from_seconds(t_cumulative + t + 1e-4 * seg);

        RCLCPP_INFO(this->get_logger(),"p position = %.3f ", p.positions[0]);
        RCLCPP_INFO(this->get_logger(),"p position = %.3f ", p.positions[1]);
        RCLCPP_INFO(this->get_logger(),"p position = %.3f ", p.positions[2]);
    ...
    if (traj.points.empty()) {
      RCLCPP_WARN(this->get_logger(), "No valid segments to publish!");
      return;
    }
...
if(request->cmd==1){
  ...
  if (model == "roarm_m2") {
    ...
    if (valid) {
      hand_node->addPoint(temp_target,3.0);
  ...
  // pose[0]= 1000*target_pose.position.x;
  // pose[1]= 1000*target_pose.position.y+30;
  // pose[2]= 1000*target_pose.position.z;

  auto near_zero = [](double v){ return std::fabs(v) < 1e-3; };

  if (!(near_zero(target_pose.position.x) &&
        near_zero(target_pose.position.y) &&
        near_zero(target_pose.position.z)))
  {
    pose[0] = 1000 * target_pose.position.x;
    pose[1] = 1000 * target_pose.position.y + 30;
    pose[2] = 1000 * target_pose.position.z;
      ...
      if (valid) {
        ...
        if (valid) {
          ...
          if (valid) {
            ...
            response->success = true;
            response->message = "PickPlaceCmd executed successfully";
          }
        }
      }
    } else if (model == "roarm_m3") {
  ...
  {
      RCLCPP_WARN(logger, "⚠️");
        response->success = false;
        response->message = "PickPlaceCmd executed failed"; 
  }
}
}
